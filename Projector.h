...
public:
	...
	/** @brief 2D resolution descriptor. */
	typename typedef EigenTypes<unsigned>::Vec2 Res2;
...
public:
	...
	Projector(unsigned resX, unsigned resY, real SAR=1);

	/** @brief Constructs the projector with the given parameters. */
	Projector(const Res2 &resolution, real SAR=1);
	...
	*/
	...
	/**
	 * @brief
	 *		Writes the current camera pose matrix, resulting from the combined
	 *		transformation @link IScannableObject::getWorldTransform
	 *		object.getWorldTransform() @endlink * @ref #viewMatrix, to a text
	 *		file in augmented 3x3|1x3 form (rotation|translation). Optionally returns the
	 *		camera pose matrix itself.
	 */
	virtual void writeCameraPose_txt (
		const std::string &filename, const IScannableObject<real> &object,
		Mat4 *cameraPose=nullptr
	) const;

	/**
	 * @brief
	 *		Writes the current @ref #projectionMatrix to a file as a series of numbers
	 *		in row-major format.
	 */
	void writeProjectionMatrix_txt (const std::string &filename) const;
	...
	virtual void setResolution (const Res2 &res);
	/** @brief Returns current contents of @ref #resolution . */
	virtual Res2 getResolution (void) const;
	/** @brief Writes current contents of @ref #resolution to vector @c out . */
	virtual void getResolution (Res2 *out) const;
