...
namespace {
	// Templated constants
	const float  _pi_flt = std::acos(float(-1));
	const double _pi_dbl = std::acos(double(-1));
	...
	{
		inline static float pi(void) {return _pi_flt;}
	};
	template <>
	struct Constants<double>
	{
		inline static double pi(void) {return _pi_dbl;}
	};
};
...
template <class flt_type>
Projector<flt_type>::Projector(real resX, real resY, real SAR)
...
template <class flt_type>
Projector<flt_type>::Projector(const Vec2 &resolution, real SAR)
...
template <class flt_type>
void Projector<flt_type>::setResolution (const Vec2 &res)
{
	resolution = res;
}
template <class flt_type>
typename EigenTypes<flt_type>::Vec2 Projector<flt_type>::getResolution (void) const
{
	return resolution;
}
template <class flt_type>
void Projector<flt_type>::getResolution (Vec2 *out) const
{
	*out = resolution;
}
